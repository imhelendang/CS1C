...
bool evensFirst(int x, int y)
{
	...
	return ascending(x, y);  // included so that incomplete lab code will compile
}
...
int main()
{
    ...
    //fcnPtr1 = &boo; // [1.4] assign fcnPtr1 to point to boo - not allowed -- fcnPtr1 has no parameters, but boo() does
    ...
    // TEMP - COMPLETE FUNCTION CALL AND UNCOMMENT LINE BELOW
    selectionSort( array, 9, evensFirst ); // [2.3] sort array via custom_sort algorithm
    printArray(array, 9);
...
// Q#4 - dPtr->function1(); // [Q#4] what happens here ... explain in WRITTEN ANSWERS below?

// The program recognizes that function1() is a virtual function. 
// Then, the program uses dPtr->__vptr to get to D2's virtual table to look up 
// which version of function1() to call in that virtual table, which has been set to D2::function1(). 
// Therefore, dPtr->function1() resolves to D2::function1()!

// Q#5

// Object slicing happens. 
// Object dSlice, derived class, has to slice off its additional attributes to form the base class object. 
